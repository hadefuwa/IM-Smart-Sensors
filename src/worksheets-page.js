    ...
    contentHtml: `
      ...
      <div class="rounded-xl border-2 border-primary/30 bg-primary/5 p-4 mt-4 space-y-3">
        ...
        <div class="space-y-2 text-sm">
          ...
          <label class="flex items-start gap-3 cursor-pointer"><input type="checkbox" class="checkbox checkbox-sm checkbox-primary mt-0.5 flex-shrink-0">
            <span><strong class="text-base-content">Port 1 — Photoelectric Sensor (RS PRO Diffuse, M18)</strong> — blue barrel sensor pointing outward, red LED on the face. It detects objects between 3 mm and 1200 mm away by bouncing its own light beam off the target — no reflector needed. IP67 rated (dust-tight and waterproof), so it survives a factory floor. IO-Link lets you adjust its sensitivity from this screen without touching the sensor.</span></label>
          <label class="flex items-start gap-3 cursor-pointer"><input type="checkbox" class="checkbox checkbox-sm checkbox-primary mt-0.5 flex-shrink-0">
    ...
    <div class="worksheets-index max-w-5xl mx-auto space-y-6 relative min-h-full py-2 rounded-2xl" style="background:linear-gradient(160deg,hsl(var(--b2)) 0%,hsl(var(--b3)) 40%,hsl(var(--p)/0.06) 100%);">
      <header class="relative rounded-2xl bg-gradient-to-r from-primary/20 via-secondary/15 to-accent/20 border-2 border-primary/30 px-6 py-6 shadow-xl">
        <div class="flex items-start gap-4 flex-wrap">
          ...
          <div>
            <div class="flex items-center gap-2 mb-1"><span class="badge badge-primary badge-outline font-mono text-xs">CP0001</span></div>
            <h1 class="text-3xl font-bold text-base-content tracking-tight">Maintenance on Smart Sensors</h1>
            <p class="mt-2 text-base-content/80 text-lg">${TOTAL} interactive worksheets — live sensor data, charts, and tasks built in.</p>
          </div>
        </div>
      </header>
